...
\label{sec:schätzung_der_irradianz}
	...
	\label{sub:fehlerabschätzung_der_irradianz}
		...
		\begin{figure}[h]
			...
			\end{subfigure}
			\caption{Die Bilder zeigen die gerenderte \enquote{Dragon}-Szene mit einer schwach variierenden Umgebungsbeleuchtung. \ref{subfig:irr-est-rc-dragon-pt} ist ein durch Path Tracing aufgenommenes Referenzbild. Für \ref{subfig:irr-est-rc-dragon-512} und \ref{subfig:irr-est-rc-dragon-4096} wurden die Irradianzen an den Eckpunkten der Mesh geschätzt und gespeichert. Zwischen den Eckpunkten fand eine lineare Interpolation statt. Die Anzahl der Samples gibt an, wie viele Stichprobenwerte verwendet wurden, um die Irradianz an jedem Punkt zu schätzen. Die Zeit entspricht der Berechnungsdauer für alle Punkte.}
			\label{fig:irr-est-rc-dragon}
		\end{figure}
		...
		\begin{figure}[h]
			...
			\end{subfigure}
			\caption{Die Bilder zeigen die gerenderte \enquote{Shaderball}-Szene analog zu Abbildung \ref{fig:irr-est-rc-dragon}. \ref{subfig:irr-est-rc-shaderball-512} zeigt, dass die Verwendung von nur $512$ Samples starke Flecken erzeugen, die erst für größere Sampleanzahlen verschwinden.}
